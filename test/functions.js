...
const insertPlace = async (attractionData) => {
  const randomRating = Math.floor(Math.random() * 5) + 1; // Generate a random rating between 1 and 5

  const res = await pool.query(
    `INSERT INTO places (name, lat, lon, image_url, road, city, state, country, rating, location) 
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, ST_SetSRID(ST_MakePoint($3, $2), 4326)) RETURNING id`,
    [
      attractionData.name, // name
      attractionData.lat, // lat
      attractionData.lon, // lon
      null, // image_url (you can update this later if needed)
      attractionData.address.road, // road
      attractionData.address.city, // city
      attractionData.address.state, // state
      attractionData.address.country, // country
...
const main = async () => {
  try {
    await pool.connect();
    const placesData = loadLocationData("../data/attractions2.json");

    for (const place of placesData) {
      await insertPlace(place);
    }

    console.log("All places have been successfully added to the database.");
  } catch (error) {
...
const fetchAttractions = async () => {
  ...
  for (const attraction of attractionCoordinates) {
    try {
      ...
      if (!addressResponse.ok) {
        throw new Error(
          `Failed to fetch address for attraction ID: ${attraction.id}`
        );
      }
      const addressData = await addressResponse.json();
      console.log("addressData: ", addressData);
  ...
  fs.writeFileSync(
    ATTRACTIONS_FILE,
    JSON.stringify(attractionsWithAddresses, null, 2)
  );
